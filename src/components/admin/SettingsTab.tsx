...
/* ── Coupons — discount codes for the /buy checkout ─────────────────
 *  The server enforces a hard 50% cap + a price floor; a code only ever
 *  MAPS to a cheaper PayPal plan, never sets a client-controlled price.
 *  This card just manages the code list. */
...
function CouponsCard({ onErr }: { onErr: (e: unknown) => void }) {
  ...
  const toggle = async (c: AdminCoupon) => {
    try {
      await adminApi('admin-set-coupon-active', { code: c.code, active: !c.active })
      await load()
    } catch (e) {
      onErr(e)
    }
  ...
  return (
    <Card title="קופונים">
      <div className="flex items-center gap-2 text-xs text-fg-muted">
        ...
        קודי הנחה לעמוד הרכישה. ההנחה חלה על כל חיי המנוי, מוגבלת ל-50%, ולא
        מצטברת עם מבצע פעיל (הלקוח מקבל את הזול מביניהם).
      </div>

      {/* create */}
      <div className="grid grid-cols-2 gap-2 rounded-xl border border-border bg-bg-elevated p-3 md:grid-cols-3">
        ...
        <label className="text-xs text-fg-secondary">
          ...
          <select
            ...
            className="mt-1 w-full rounded-lg border border-border bg-card px-2 py-2 text-sm text-fg focus:outline-none focus:ring-1 focus:ring-primary"
          >
